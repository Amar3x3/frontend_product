...
export default class Career extends Component {
    ...
    handleSubmit(event){
        ...
        var id;
        axios.post('http://127.0.0.1:8080/careercontent/apply', body)
            .then(function (response) {
                id =response.data._id;
                alert('Submitted successfully!');
                const formData = new FormData();                 
                formData.append('file',file,id+'.pdf');
                return axios.post('http://127.0.0.1:8080/careercontent/upload', formData, {
                    // headers: {
                ...
                return axios.post('http://127.0.0.1:8080/emailcontent/sendemail', body)
                 }).then((response)=>{
                    console.log(response);
                 })
            .catch((error)=> {
